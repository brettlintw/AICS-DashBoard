...
# 1. 頁面設定
st.set_page_config(layout="wide", page_title="AICS 北美部署決策中心 V8.10 (PPTX導出加強版)")
st.title("🌐 AICS 北美部署決策中心 (V8.10 正式版)")
...
if uploaded_file:
    ...
    st.session_state.charts_to_export = [fig_map]
    ...
    def render_analysis_section(data, dimension, title_name):
        ...
        st.session_state.charts_to_export.append(fig) # 加入導出清單
        ...
        if st.checkbox(f"顯示數據列表", key=f"ch_{dimension}"):
            ...
            if mode == "全時間段彙總" and sort != "預設": pivot = pivot.sort_values(by=pivot.columns[0], ascending=(sort=="由小至大"))
            total = pivot.sum(axis=0); total.name = '合計'
            pivot = pd.concat([total.to_frame().T, pivot])
            st.dataframe(pivot.style.apply(lambda row: ['color: blue; font-weight: bold;' if row.name == '合計' else ''] * len(row), axis=1), use_container_width=True)
