...
# 頁面設定
st.set_page_config(layout="wide", page_title="AICS 北美部署決策中心 V8.13")
st.markdown("""<style>.stDataFrame table td, .stDataFrame table th { white-space: nowrap !important; }</style>""", unsafe_html=True)

st.title("🌐 AICS 北美部署決策中心 (V8.13 終極版)")
...
if uploaded_file:
    ...
    def render_analysis_section(data, dimension, title_name):
        ...
        if st.checkbox(f"顯示數據列表", key=f"ch_{dimension}"):
            ...
            # Step 2: 排序處理 (僅針對數據主體，避開合計行)
            if mode == "全時間段彙總" and sort != "預設":
                pv = pv.sort_values(by=pv.columns[0], ascending=(sort=="由小至大"))
            
            # Step 3: 計算行/列合計 (最後才算)
            pv['行合計'] = pv.sum(axis=1)
            col_totals = pv.sum(axis=0)
            col_totals.name = '列合計'
            pv = pd.concat([pv, col_totals.to_frame().T])
