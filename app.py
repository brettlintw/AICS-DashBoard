...
st.set_page_config(layout="wide", page_title="AICS 北美部署決策中心")
st.title("🌐 AICS 北美部署決策中心 (V8.10 完整版)")
...
if uploaded_file:
    ...
    fig_map = go.Figure()
    # 畫州代碼
    fig_map.add_trace(go.Scattergeo(lon=[US_STATES_COORDS[s][1] for s in US_STATES_COORDS], lat=[US_STATES_COORDS[s][0] for s in US_STATES_COORDS], text=list(US_STATES_COORDS.keys()), mode='text', textfont=dict(size=9, color='red'), showlegend=False))
    # 畫數據氣泡
    ...
    fig_map.update_layout(geo=dict(scope='usa', fitbounds="locations"), height=500, margin={"l": 0, "r": 150, "t": 0, "b": 0}, legend=dict(x=1.0, y=0.5))
    ...
    for dim, name in dims:
        ...
        with c2:
            ...
            if chart_type == "柱狀圖": fig = px.bar(df_g, x=dim if mode=="全時間段" else "Month-Date", y='Outbound Qty (Item)', color=dim, text='Outbound Qty (Item)')
