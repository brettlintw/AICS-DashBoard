...
st.title("🌐 AICS 北美部署決策中心 (V8.10 完整地圖版)")
...
if uploaded_file:
    ...
    fig_map.update_layout(geo=dict(scope='north america', projection_type='albers usa'), height=600, margin={"r":0,"t":0,"l":0,"b":0})
    ...
    for dim, name in [('Machine Type', '設備維度'), ('Field', '場域維度'), ('Area', '區域維度'), ('Company', '客戶維度'), ('Device/Platform', '平台維度')]:
        ...
        with c1:
            fig = px.bar(df_g, x=dim, y='Outbound Qty (Item)', text='Outbound Qty (Item)')
        ...
        try:
            ...
            for dim, name in dims:
                ...
                try:
                    ...
                    slide.shapes.add_picture(io.BytesIO(img_bytes), Inches(0.5), Inches(1), width=Inches(9), height=Inches(3.5))
