...
# 1. 頁面設定
st.set_page_config(layout="wide", page_title="AICS 北美部署決策中心 V7.4")
...
st.title("🌐 AICS 北美部署決策中心 (V7.4 月份刻度補齊版)")
...
if uploaded_file:
    ...
    # 【關鍵修改】：改用 datetime 格式處理月份，確保 X 軸完整顯示
    ...
    def render_analysis_section(data, dimension, title_name):
        ...
        if st.checkbox(f"顯示數據列表", key=f"ch_{dimension}"):
            pivot = data.pivot_table(index=dimension, columns=x_col if mode=="月份推移" else None, values='Outbound Qty (Item)', aggfunc='sum', fill_value=0, margins=True, margins_name='合計')
