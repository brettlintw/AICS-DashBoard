...
st.title("🌐 AICS 北美部署決策中心 (V8.10 穩定導出版)")
    ...
    if st.sidebar.button("📊 導出完整戰情室報表"):
        try:
            ...
            for dim, name in dimensions:
                ...
                table = slide.shapes.add_table(rows + 1, cols, Inches(1), Inches(2), Inches(8), Inches(3)).table
                for i, col_name in enumerate(df_g.columns): table.cell(0, i).text = col_name
                for r in range(rows):
            ...
            buf = io.BytesIO()
            prs.save(buf)
            st.sidebar.download_button("下載正式報表 PPTX", buf.getvalue(), "Tactical_Report.pptx")
