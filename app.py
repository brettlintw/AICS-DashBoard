...
st.markdown("""
    ...
    @media print {
        ...
        /* 強制圖片渲染品質 */
        img { image-rendering: -webkit-optimize-contrast !important; filter: contrast(110%) brightness(100%) !important; }
        body { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
    }
    </style>
""", unsafe_allow_html=True)

st.title("🌐 AICS 北美部署決策中心 (V8.23 圖像渲染優化版)")
...
# 高解析度渲染函數
def render_static_chart(fig):
    img_bytes = pio.to_image(fig, format="png", scale=2, width=1200, height=600)
...
if uploaded_file:
    ...
    if st.sidebar.button("🖨️ 匯出高解析度 PDF"):
    ...
    for col, label in dims:
        selected = st.sidebar.multiselect(f"{label}篩選", df[col].unique().tolist())
