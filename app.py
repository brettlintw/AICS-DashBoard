...
st.title("🌐 AICS 北美部署決策中心 (V8.10 最終完整版)")
...
if uploaded_file:
    ...
    for state, coords in US_STATES_COORDS.items():
        fig_map.add_trace(go.Scattergeo(lon=[coords[1]], lat=[coords[0]], text=[state], mode='text', textfont=dict(size=10, color='red'), hoverinfo='text'))
        
    # 標示數據點
    for m in selected_machines:
        m_df = f_df[f_df['Machine Type'] == m].groupby('State Code')['Outbound Qty (Item)'].sum().reset_index()
        fig_map.add_trace(go.Scattergeo(locations=m_df['State Code'], locationmode="USA-states", marker=dict(size=m_df['Outbound Qty (Item)']*0.5, opacity=0.6), name=m))
    ...
    for dim, name in dims:
        ...
        st.plotly_chart(px.bar(df_g, x=dim, y='Outbound Qty (Item)', text='Outbound Qty (Item)'), use_container_width=True)
