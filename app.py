...
st.title("🌐 AICS 北美部署決策中心 (V8.10 完整匯出版)")
...
if uploaded_file:
    ...
    with col_exp2:
        if st.button("📄 導出 PDF"):
            ...
            pdf.cell(200, 10, txt="AICS 北美部署決策中心報告", ln=True, align='C')
            for dim, name in dims:
                pdf.add_page()
                pdf.set_font("Arial", 'B', 12)
                pdf.cell(200, 10, txt=f"分析維度: {name}", ln=True)
                df_g = f_df.groupby(dim)[['Outbound Qty (Item)']].sum().reset_index()
                pdf.set_font("Arial", '', 10)
                for index, row in df_g.iterrows():
                    pdf.cell(200, 10, txt=f"{row[dim]}: {row['Outbound Qty (Item)']}", ln=True)
