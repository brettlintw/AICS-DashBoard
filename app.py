...
if uploaded_file:
    ...
    fig_map.update_layout(geo=dict(scope='usa', fitbounds="locations"), height=500, margin={"l": 0, "r": 50, "t": 0, "b": 0}, legend=dict(x=1.0, y=0.5))
    st.plotly_chart(fig_map, use_container_width=True)

    # 2. 維度分析
    for dim, name in [('Machine Type', '設備維度'), ('Field', '場域維度'), ('Area', '區域維度'), ('Company', '客戶維度')]:
        st.markdown("---")
        st.subheader(f"📈 {name}")
