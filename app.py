...
# 1. 頁面設定
st.set_page_config(layout="wide", page_title="AICS 北美部署決策中心 V7.5")
st.title("🌐 AICS 北美部署決策中心 (V7.5 穩定修復版)")
...
if uploaded_file:
    ...
    f_df['Month-Date'] = f_df['Date(出庫)'].dt.to_period('M').dt.to_timestamp()
    ...
    def render_analysis_section(data, dimension, title_name):
        ...
        if st.checkbox(f"顯示數據列表", key=f"ch_{dimension}"):
            # 使用更安全的 pivot 方式
            pivot = data.groupby([dimension, 'Month-Date'] if mode=="月份推移" else dimension)['Outbound Qty (Item)'].sum().unstack(fill_value=0)
            if mode == "月份推移": pivot['合計'] = pivot.sum(axis=1)
            else: pivot = pivot.to_frame('Outbound Qty (Item)'); pivot.loc['合計'] = pivot.sum()
